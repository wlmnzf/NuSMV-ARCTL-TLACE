...
static char rcsid[] UTIL_UNUSED = "$Id: BddEnc.c,v 1.1.2.55.2.6 2004/07/29 14:43:41 nusmv Exp $";
...
void BddEnc_merge(BddEnc_ptr self)
{
  ...
  while (! ListIter_is_end(iter)) {
    node_ptr constant = NodeList_get_value_at(list, iter);
			...
			const VarOrderingType dump_type)
{
  ...
  while (! ListIter_is_end(iter)) {
    node_ptr name = NodeList_get_value_at(ordering, iter);
  ...
  while (! ListIter_is_end(iter)) {
    node_ptr name = NodeList_get_value_at(scalar_vars, iter);
    print_node(oof, name);
    fprintf(oof, "\n"); 
...
static add_ptr bdd_enc_get_vars_list_mask(BddEnc_ptr self, NodeList_ptr vars)
{
  ...
  while (!ListIter_is_end(iter)) {
    ...
    var = NodeList_get_value_at(vars, iter);
