...
static char rcsid[] UTIL_UNUSED = "$Id: grammar.y,v 1.19.4.10 2004/05/27 14:23:42 nusmv Exp $";
...
/*
  ...
  TOK_STEP TOK_RESET TOK_ASYNC TOK_MODTYPE TOK_LAMBDA TOK_CONTEXT TOK_EU TOK_AU TOK_EW TOK_AW TOK_EBU TOK_ABU TOK_MINU TOK_MAXU
...
%left TOK_ASYNC TOK_MODULE TOK_PROCESS TOK_MODTYPE TOK_LAMBDA TOK_CONTEXT TOK_EU TOK_AU TOK_EW TOK_AW TOK_EBU TOK_ABU TOK_MINU TOK_MAXU
...
%left  TOK_AND
%left  TOK_NOT
%left  TOK_EX TOK_AX TOK_EAX TOK_AAX TOK_EF TOK_AF TOK_EG TOK_AG TOK_EAF TOK_AAF TOK_EAG TOK_AAG TOK_REACHABLE TOK_START TOK_EE TOK_AA TOK_EEA TOK_AAA TOK_SINCE TOK_UNTIL TOK_UNLESS TOK_TRIGGERED TOK_RELEASES TOK_EBF TOK_EBG TOK_ABF TOK_ABG TOK_BUNTIL TOK_MMIN TOK_MMAX
...
context       : TOK_ATOM                      { $$ = find_atom($1); }
              ...
              | context TOK_LB simple_expr TOK_RB { $$ = find_node(ARRAY, $1, $3); }
              ;    
...
action_expr	  : TOK_LP simple_expr TOK_RP { $$ = $2; };
...
/*                { $$ = new_node(SIGMA, new_node(EQUAL, $3, $5), $7); } */
              ...
              | TOK_EAX action_expr ctl_expr { $$ = new_node(EAX, $2, $3); }
              | TOK_AAX action_expr ctl_expr { $$ = new_node(AAX, $2, $3); }
              ...
              | TOK_EAF action_expr ctl_expr { $$ = new_node(EAF, $2, $3); }
              | TOK_AAF action_expr ctl_expr { $$ = new_node(AAF, $2, $3); }
              | TOK_EAG action_expr ctl_expr { $$ = new_node(EAG, $2, $3); }
              | TOK_AAG action_expr ctl_expr { $$ = new_node(AAG, $2, $3); }
              | TOK_REACHABLE simple_expr { $$ = new_node(REACHABLE, $2, Nil); }
              | TOK_START { $$ = new_node(START, Nil, Nil); }
	      ...
	      | TOK_AA TOK_LB ctl_expr TOK_UNLESS ctl_expr TOK_RB { $$ = new_node(AW, $3, $5); }
	      | TOK_EE TOK_LB ctl_expr TOK_UNLESS ctl_expr TOK_RB { $$ = new_node(EW, $3, $5); }
	      ...
	      | TOK_EEA action_expr TOK_LB ctl_expr TOK_UNTIL ctl_expr TOK_RB
                       { $$ = new_node(EAU, $2, new_node(EU, $4, $6));
		       }
	      | TOK_AAA action_expr TOK_LB ctl_expr TOK_UNTIL ctl_expr TOK_RB
                       { $$ = new_node(AAU, $2, new_node(EU, $4, $6)); }
          | TOK_EEA action_expr TOK_LB ctl_expr TOK_UNLESS ctl_expr TOK_RB
                       { $$ = new_node(EAW, $2, new_node(EU, $4, $6));
		       }
	      | TOK_AAA action_expr TOK_LB ctl_expr TOK_UNLESS ctl_expr TOK_RB
                       { $$ = new_node(AAW, $2, new_node(EU, $4, $6)); }
