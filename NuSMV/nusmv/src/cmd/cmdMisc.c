...
static char rcsid[] UTIL_UNUSED = "$Id: cmdMisc.c,v 1.12.2.1 2003/08/07 12:04:06 nusmv Exp $";
...
/**Function********************************************************************
  ...
  CommandSynopsis    [Merely echoes the arguments]
  ...
  char ** argv)
{
  ...
  for(i = 1; i < argc; i++) {
    fprintf(nusmv_stdout, "%s ", argv[i]);
  }
  fprintf(nusmv_stdout, "\n");
  return 0;

  usage:
  fprintf(nusmv_stderr, "usage: echo [-h] string \n");
  fprintf(nusmv_stderr, "   -h \t\tPrints the command usage.\n");
  return (1);  
}
