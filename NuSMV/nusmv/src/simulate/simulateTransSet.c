...
static char rcsid[] UTIL_UNUSED = "$Id: simulateTransSet.c,v 1.1.2.8 2004/05/26 07:46:37 nusmv Exp $";
...
/**Struct**********************************************************************
  ...
  transitions. Used during simulation]
			...
			int next_states_count)
{
  ...
  nusmv_assert(next_states_count > 0);

  self->enc = enc;
  self->dd = BddEnc_get_dd_manager(enc);
  self->senc = BddEnc_get_symbolic_encoding(enc);

  self->next_states_num = next_states_count;
  self->next_states_array = ALLOC(bdd_ptr, next_states_count);
