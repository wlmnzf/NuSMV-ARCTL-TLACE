...
static char rcsid[] UTIL_UNUSED = "$Id: optCmd.c,v 1.48.4.15.2.4 2004/07/28 14:03:50 nusmv Exp $";
...
options_ptr init_options()
{
  ...
  /* 29/04/10 sbusard */
  opt->tlaces_output_file = NIL(char);
  opt->tlaces_explain_level = 0;
...
/* ----- 29/04/10 sbusard -------------------------------------------------- */
void set_tlaces_output_file(options_ptr opt, char * str)
{
  nusmv_assert(opt != NULL);
  if (util_is_string_null(str))  opt->tlaces_output_file = NIL(char);
  else opt->tlaces_output_file = util_strsav(str);
}
void reset_tlaces_output_file(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_output_file = NIL(char);
}
char * get_tlaces_output_file(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return(opt->tlaces_output_file);
}

/* 05/04/11 sbusard */
void set_tlaces_explain_level_activate(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_explain_level |= TLACES_EXPLAIN_LEVEL_ACTIVATE;
}

void set_tlaces_explain_level_ex(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_explain_level |= TLACES_EXPLAIN_LEVEL_EX;
}
void set_tlaces_explain_level_eu(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_explain_level |= TLACES_EXPLAIN_LEVEL_EU;
}
void set_tlaces_explain_level_eg(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_explain_level |= TLACES_EXPLAIN_LEVEL_EG;
}

/* 05/04/11 sbusard */
void unset_tlaces_explain_level_activate(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  opt->tlaces_explain_level = 0;
}

void unset_tlaces_explain_level_ex(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  if(opt_tlaces_explain_level_ex(opt))
	  opt->tlaces_explain_level -= TLACES_EXPLAIN_LEVEL_EX;
}
void unset_tlaces_explain_level_eu(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  if(opt_tlaces_explain_level_eu(opt))
	  opt->tlaces_explain_level -= TLACES_EXPLAIN_LEVEL_EU;
}
void unset_tlaces_explain_level_eg(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  if(opt_tlaces_explain_level_eg(opt))
	  opt->tlaces_explain_level -= TLACES_EXPLAIN_LEVEL_EG;
}
boolean opt_tlaces_explain_level(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return((opt->tlaces_explain_level > 0));
}

/* 05/04/11 sbusard */
boolean opt_tlaces_explain_level_activate(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return((opt->tlaces_explain_level & TLACES_EXPLAIN_LEVEL_ACTIVATE));
}

boolean opt_tlaces_explain_level_ex(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return((opt->tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EX));
}
boolean opt_tlaces_explain_level_eu(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return((opt->tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EU));
}
boolean opt_tlaces_explain_level_eg(options_ptr opt)
{
  nusmv_assert(opt != NULL);
  return((opt->tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EG));
}
/* ----- sbusard ----------------------------------------------------------- */


/* ----- 08/01/11 sbusard -------------------------------------------------- */
void set_tlaces_depth(options_ptr opt, int depth)
{
	nusmv_assert(opt != NULL);
	opt->tlaces_depth = depth;
}
void unset_tlaces_depth(options_ptr opt)
{
	nusmv_assert(opt != NULL);
	opt->tlaces_depth = -1;
}
int get_tlaces_depth(options_ptr opt)
{
	nusmv_assert(opt != NULL);
	return opt->tlaces_depth;
}
/* ----- sbusard ----------------------------------------------------------- */
...
# endif
    ...
    /* 29/04/10 sbusard */
	if (strcmp(argv[1], TLACES_OUTPUT_FILE) == 0) {
	  if (util_is_string_null(flag_value)) {
		set_tlaces_output_file(options, NULL);
	  }
	  else {
		set_tlaces_output_file(options, util_strsav(flag_value));
	  }
	  return 0;
	}
	if (strcmp(argv[1], TLACES_EXPLAIN_LEVEL) == 0) {
	  char *err_occ[1];
	  int cur_tlaces_explain_level;

	  err_occ[0] = "";
	  cur_tlaces_explain_level = strtol(flag_value,err_occ, 10);
	  if (strcmp(err_occ[0], "") != 0) {
		fprintf(nusmv_stderr, "Error: \"%s\" is not a valid value for the \"%s\" option.\n", err_occ[0], TLACES_EXPLAIN_LEVEL);
		return 1;
	  }
	  if(cur_tlaces_explain_level == 0)
		  set_tlaces_explain_level_activate(options);
	  if(cur_tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EX)
		  set_tlaces_explain_level_ex(options);
	  if(cur_tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EU)
		  set_tlaces_explain_level_eu(options);
	  if(cur_tlaces_explain_level & TLACES_EXPLAIN_LEVEL_EG)
		  set_tlaces_explain_level_eg(options);

	  return 0;
	}

	/* 01/08/11 sbusard */
	if (strcmp(argv[1], TLACES_DEPTH) == 0) {
	  char *err_occ[1];
	  int cur_tlaces_depth;

	  err_occ[0] = "";
	  cur_tlaces_depth = strtol(flag_value,err_occ, 10);
	  if (strcmp(err_occ[0], "") != 0) {
		fprintf(nusmv_stderr, "Error: \"%s\" is not a valid value for the \"%s\" option.\n", err_occ[0], TLACES_DEPTH);
		return 1;
	  }
	  set_tlaces_depth(options, cur_tlaces_depth);
...
int CommandUnsetVariable(int  argc, char ** argv)
{
  ...
  /* 29/04/10 sbusard */
	if (strcmp(argv[1], TLACES_OUTPUT_FILE) == 0) {
	  reset_tlaces_output_file(options);
	  return 0;
	}
	if (strcmp(argv[1], TLACES_EXPLAIN_LEVEL) == 0) {
	  unset_tlaces_explain_level_ex(options);
	  unset_tlaces_explain_level_eu(options);
	  unset_tlaces_explain_level_eg(options);
	  return 0;
	}

  /* 01/08/11 sbusard */
	if (strcmp(argv[1], TLACES_DEPTH) == 0) {
	  unset_tlaces_depth(options);
	  return 0;
	}
...
static void print_options(options_ptr opt, FILE * fn)
{
  ...
  /* 29/04/10 sbusard */
    fprintf(fn, "%s\t\t\"%s\"\n",   TLACES_OUTPUT_FILE,
            (opt->tlaces_output_file != (char*)NULL)
            ? opt->tlaces_output_file : OPT_USER_POV_NULL_STRING);
    int explainlevel = opt->tlaces_explain_level;
    if (explainlevel == 0) {
    	explainlevel = -1;
    } else if (explainlevel & TLACES_EXPLAIN_LEVEL_ACTIVATE) {
    	explainlevel = 0;
    }
    fprintf(fn, "%s\t\t\"%d\"\n", TLACES_EXPLAIN_LEVEL, explainlevel);

    /* 01/08/11 sbusard */
    fprintf(fn, "%s\t\t\t\"%d\"\n", TLACES_DEPTH, opt->tlaces_depth);
}
...
static char* remove_non_existant_pps(const char* pp_list)
{
  ...
  pp_list_copy = ALLOC(char, len+1);
  nusmv_assert(pp_list_copy != (char*) NULL);

  tmp_list = ALLOC(char, len+1);
  nusmv_assert(tmp_list != (char*) NULL);
  strncpy(tmp_list, pp_list, len+1);
  tmp_pp = strtok(tmp_list, " \t\n\r");
  while (tmp_pp != (char*) NULL) {
    int tmp_pp_len;
    tmp_pp_len = strlen(tmp_pp); 

    if (strcmp(pp_list_copy, "") == 0) {
      strncpy(pp_list_copy, tmp_pp, tmp_pp_len+1);
    }
    else {
      strncat(pp_list_copy, " ", 1);
      strncat(pp_list_copy, tmp_pp, tmp_pp_len);
    }
    tmp_pp = strtok(NULL, " \t\n\r");
  } /* loop */

  copy_len = strlen(pp_list_copy) + 1;
  pp = strtok(pp_list_copy, " \t\n\r");
  copy_len = copy_len - (strlen(pp) + 1);
...
# endif
  ...
  while (pp != (char*) NULL) {
	...
	( Utils_file_exists_in_paths(pp_filename, open_path, delimiters) ||
          Utils_file_exists_in_paths(pp_filename, paths, delimiters) )) {
      if (strncmp(valid_pps, "", 1) == 0) {
	strncat(valid_pps, pp, strlen(pp));
      }
      else {
	strncat(valid_pps, " ", 1);
	strncat(valid_pps, pp, strlen(pp));
