...
static char rcsid[] UTIL_UNUSED = "$Id: mcExplain.c,v 1.11.4.33 2004/05/11 08:14:40 nusmv Exp $";
...
//JVANDERM's CHANGES BEGIN
static node_ptr Extend_trace_with_constrained_state_input_pair(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path,
		bdd_ptr starting_state, bdd_ptr next_states, bdd_ptr si, const char * comment);
//JVANDERM's CHANGES END

/* 19/01/11 sbusard */
static node_ptr
Extend_trace_with_constrained_states_inputs_pair ARGS ((BddFsm_ptr fsm, BddEnc_ptr enc,
                                            node_ptr path,
                                            bdd_ptr starting_state,
                                            bdd_ptr next_states,
                                            bdd_ptr si,
...
/**Function********************************************************************
  ...
  Synopsis           [This function computes a path that is a witness
                      for <i>REACHABLE(act)</i>.]

  Description        [This function finds a path that is a witness for
  <i>REACHABLE(act)</i>.  <code>path</code> is a BDD representing the first
  state of the path.  It is a state from which the example can be found.
  The returned path is Nil, or the reversed witness, extended with path.]

  SideEffects        []

  SeeAlso            [explain_tlace]

******************************************************************************/
node_ptr reachable_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr act) {

	  bdd_ptr reachable_states_bdd;
	  bdd_ptr from_lower_bound;   /* the frontier */
	  bdd_ptr invars, result;

	  node_ptr itlist, old_itlist;
	  bdd_ptr state_reachable;
	  bdd_ptr pre;
	  bdd_ptr state, inputs, input;

	  if(path == Nil) {
		  return path;
	  }

	  reachable_states_bdd = BddFsm_get_init(fsm);
	  invars = BddFsm_get_state_constraints(fsm);
	  bdd_and_accumulate(dd_manager, &reachable_states_bdd, invars);
	  bdd_free(dd_manager, invars);

	  from_lower_bound = BddFsm_get_init(fsm);

	  itlist = cons((node_ptr) bdd_dup(reachable_states_bdd), Nil);

	  if (opt_verbose_level_gt(options, 1)) {
	    fprintf(nusmv_stderr, "\ncomputing witness for REACHABLE(act)\n");
	  }

	  while (bdd_isnot_zero(dd_manager, from_lower_bound)) {

		  // If state is in reachable
		  state_reachable = bdd_and(dd_manager, (bdd_ptr) car(path), reachable_states_bdd);
		  if(bdd_isnot_zero(dd_manager, state_reachable)) {

			  // Discard first (R)
			  bdd_free(dd_manager, (bdd_ptr) car(itlist));
			  old_itlist = itlist;
			  itlist = cdr(itlist);
			  free_node(old_itlist);

			  // Get the path
			  while(itlist != Nil) {
				  pre = BddFsm_get_backward_image(fsm, bdd_and(dd_manager,
		  						 BDD_STATES((bdd_ptr) car(path)), (bdd_ptr) act));
				  bdd_and_accumulate(dd_manager, &pre, (bdd_ptr) car(itlist));
				  state = BddEnc_pick_one_state(enc, pre);

				  inputs = BddFsm_states_to_states_get_inputs(fsm,
		  					state, (bdd_ptr) car(path));
				  bdd_and_accumulate(dd_manager, &inputs, act);
				  input = BddEnc_pick_one_input(enc, inputs);

				  bdd_free(dd_manager, inputs);

				  path = cons((node_ptr) state, cons((node_ptr) input, path));
	    ...
	    {
	      BddStates img =
		BddFsm_get_forward_image(fsm,
					 bdd_and(dd_manager,
						 BDD_STATES(from_lower_bound),(bdd_ptr) act));
	    ...
	    /* updates from_lower_bound */
	    {
	      ...
	      bdd_free(dd_manager, from_lower_bound);
	      from_lower_bound = bdd_and(dd_manager,
	    ...
	    itlist = cons((node_ptr) bdd_dup(reachable_states_bdd), itlist);
...
node_ptr ex_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr f) {
	bdd_ptr si = eval_spec(fsm, enc, one_number, Nil);
...
******************************************************************************/
//JVANDERM's CHANGES BEGIN
node_ptr eu_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr f, bdd_ptr g) {
	bdd_ptr si = eval_spec(fsm, enc, one_number, Nil);
...
node_ptr eau_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr f, bdd_ptr g, bdd_ptr si) {
	if (path != Nil) {
		...
		while (bdd_isnot_zero(dd_manager, new)) {
					...
					if (opt_use_reachable_states(options)) {
						...
						if (bdd_is_zero(dd_manager, reachable_x)) {
							...
							BddEnc_print_bdd_begin(enc,
									NodeList_to_node_ptr(Encoding_get_all_model_symbols_list(senc)),
									false);
			...
			tmp_2 = BddFsm_get_constrained_forward_image(fsm, tmp_1, si);
			bdd_free(dd_manager, tmp_1);
			bdd_free(dd_manager, Y);
			...
			*/
			...
			witness_path = Extend_trace_with_constrained_states_inputs_pair(fsm, enc, witness_path,
															  (bdd_ptr) car(witness_path), Z, si,
															  "eu_explain: (1).");
		} /* while (new != zero) */
...
******************************************************************************/
node_ptr eg_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr witness_path, bdd_ptr arg_g) {
	bdd_ptr g;
	bdd_ptr eg_si_g;
	bdd_ptr eg_g;
	bdd_ptr tmp_1;
...
node_ptr eag_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr f, bdd_ptr a) {
	...
	while(true) {
		eaxs = eax(fsm, (bdd_ptr) car(path), a);
                              ...
                              node_ptr formula_expr, node_ptr context)
{
  ...
  case EX:
    a1 = eval_spec(fsm, enc, car(formula_expr), context);
  ...
  case EU:
    a1 = eval_spec(fsm, enc, car(formula_expr), context);
    a2 = eval_spec(fsm, enc, cdr(formula_expr), context);
    set_the_node(formula_expr);
  ...
  /* 01/08/11 sbusard */
  case EW:
      /* E[f W g] and !A[!g U (!g & !f)] are equivalent. */
      return (explain_recur(fsm, enc, path, find_node
			  (NOT, find_node
					  (AU, find_node
							  (NOT, cdr(formula_expr), Nil), find_node
							  (AND, find_node
									  (NOT, car(formula_expr), Nil), find_node
									  (NOT, cdr(formula_expr), Nil)
							  )
					  ),Nil),
  			  context));
  case AW:
        /* A[f W g] and !E[!g U (!g & !f)] are equivalent. */
      return (explain_recur(fsm, enc, path, find_node
			  (NOT, find_node
					  (EU, find_node
							  (NOT, cdr(formula_expr), Nil), find_node
  ...
  case EBU:
    a1 = eval_spec(fsm, enc, car(car(formula_expr)), context);
    a2 = eval_spec(fsm, enc, cdr(car(formula_expr)), context);
    {
  ...
  case ATOM:
    {
      ...
      add_ptr  c_add = BddEnc_constant_to_add(enc, find_atom(formula_expr));
      boolean is_const = (c_add != (add_ptr) NULL);
      boolean is_decl;
      is_decl = Encoding_is_symbol_declared(BddEnc_get_symbolic_encoding(enc),
					    name);

      if (is_const) { add_free(BddEnc_get_dd_manager(enc), c_add); }
  ...
  case ARRAY:
    {
      node_ptr v;
      v = Encoding_lookup_symbol(BddEnc_get_symbolic_encoding(enc),
				 CompileFlatten_resolve_name(formula_expr,
							     context));

      if (v != Nil) return explain_recur(fsm, enc, path, v, context);
      return Nil;
    }

  // TO ADD COMMENTS
  // JVANDERM's CHANGE (2007 09 06)	BEGIN
  case EAU: {
	  a1 = eval_spec(fsm, enc, car(formula_expr), context);
	  a2 = eval_spec(fsm, enc, car(cdr(formula_expr)), context);
	  a3 = eval_spec(fsm, enc, cdr(cdr(formula_expr)), context);
	  set_the_node(formula_expr);
	  new_path = eau_explain(fsm, enc, path, a2, a3, a1);
  ...
  case EAX: {
	  ...
	  a2 = eval_spec(fsm, enc, cdr(formula_expr), context);
	  set_the_node(formula_expr);
	  new_path = eax_explain(fsm, enc, path, a2, a1);
	  bdd_free(dd_manager, a1);
	  bdd_free(dd_manager, a2);
	  if (new_path != Nil) {
		  node_ptr q = explain_recur(fsm, enc, new_path, cdr(formula_expr), context);
  ...
  // JVANDERM's CHANGE (2007 09 06)	END

  // JVANDERM's CHANGE (2007 09 07)	START
  case EAG: {
	    a1 = eval_spec(fsm, enc, car(formula_expr), context);
	    a2 = eval_spec(fsm, enc, cdr(formula_expr), context);
	    set_the_node(formula_expr);
	    new_path = eag_explain(fsm, enc, path, a2, a1);
	    bdd_free(dd_manager, a2);
	    bdd_free(dd_manager, a1);
	    return new_path;
  }
  // JVANDERM's CHANGE (2007 09 07)	END

	/* 19/01/11 sbusard */
	case EAF:
	{
		node_ptr act = car(formula_expr);
		node_ptr phi = cdr(formula_expr);
		return explain_recur(fsm, enc, path, find_node(EAU, act, find_node(
				EU, one_number, phi)), context);
	}

	case AAU:
	{
		node_ptr act = car(formula_expr);
		node_ptr f = car(cdr(formula_expr));
		node_ptr g = cdr(cdr(formula_expr));
	    return (explain_recur(fsm, enc, path,
	    		find_node(NOT,
	    			find_node(OR,
	    				find_node(EAU, act,
	    					find_node(EU,
	    						find_node(NOT, g, Nil),
	    						find_node(AND, find_node(NOT, f, Nil),
	    							find_node(NOT, g, Nil)))),
	    				find_node(EAG, act, find_node(NOT, g, Nil))), Nil),
				  context));
	}
	...
	case AAX:
	{
		...
		return explain_recur(fsm, enc, path,
				find_node(NOT,
					find_node(EAX, act, find_node(NOT, phi, Nil)), Nil),
				context);
	}
	...
	case AAG:
	{
		...
		return explain_recur(fsm, enc, path,
				find_node(NOT,
					find_node(EAF, act, find_node(NOT, phi, Nil)), Nil),
				context);
	}
	...
	case AAF:
	{
		...
		return explain_recur(fsm, enc, path,
				find_node(NOT,
					find_node(EAG, act, find_node(NOT, phi, Nil)), Nil),
				context);
	}
	...
	case REACHABLE:
	{
		...
		bdd_ptr actions = eval_spec(fsm, enc, car(formula_expr), context);
		new_path = reachable_explain(fsm, enc, path, actions);
                            ...
                            const char * varname)
{
  ...
  for (i = 0; i < l; i++) {
    ...
    must_abort = (bdd_is_zero(dd, elem));
    if ( must_abort ) {
