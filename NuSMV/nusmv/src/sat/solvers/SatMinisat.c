...
static char rcsid[] UTIL_UNUSED = "$Id: SatMinisat.c,v 1.1.2.4 2004/07/28 17:19:58 nusmv Exp $";
...
sat_minisat_solve_groups(const SatIncSolver_ptr solver, const lsList groups)
{
  ...
  /* add the permanent group to the list of groups being solved */
  if (!sat_solver_BelongToList(groups, (lsGeneric)permanentGroup) ) {
    lsNewEnd(groups, (lsGeneric)permanentGroup, 0);
  }
