...
static char rcsid[] UTIL_UNUSED = "$Id: SatZchaff.c,v 1.1.2.4 2004/07/28 17:19:58 nusmv Exp $";
...
int sat_zchaff_cnfLiteral2zchaffLiteral(SatZchaff_ptr self, int cnfLiteral)
{
  ...
  zchaffVar = (int)find_assoc(self->cnfVar2zchaffVar, (node_ptr)cnfVar);
 
  if ((int)Nil == zchaffVar) { 
    /* create a new internal var and associate with cnf */
    zchaffVar = SAT_AddVariable(self->zchaffSolver);
    insert_assoc(self->cnfVar2zchaffVar, (node_ptr)cnfVar, (node_ptr)zchaffVar);
    insert_assoc(self->zchaffVar2cnfVar, (node_ptr)zchaffVar, (node_ptr)cnfVar);
  }
...
int sat_zchaff_zchaffLiteral2cnfLiteral(SatZchaff_ptr self, int zchaffLiteral)
{
  ...
  int cnfVar = (int)find_assoc(self->zchaffVar2cnfVar, (node_ptr)zchaffVar);
		    ...
		    SatSolverGroup group)
{
  ...
  lsForEachItem( Be_Cnf_GetClausesList(cnfProb), genClause, clause) {
    ...
    lsForEachItem(clause, genLiteral, literal) {
      zchaffClause[literalNumber] = sat_zchaff_cnfLiteral2zchaffLiteral(self,
								     literal);
      ++literalNumber;
    }
...
sat_zchaff_solve_groups(const SatIncSolver_ptr solver, const lsList groups)
{
  ...
  /* add the permanent  group */
  if (!sat_solver_BelongToList(groups, (lsGeneric)permanentGroup) ) {
    lsNewBegin(groups, (lsGeneric)permanentGroup, 0);
  }
