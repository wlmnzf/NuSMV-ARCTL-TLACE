...
static char rcsid[] UTIL_UNUSED = "$Id: BddFsm.c,v 1.1.2.44 2004/05/28 13:19:54 nusmv Exp $";
...
bdd_fsm_get_backward_si_projection ARGS((BddFsm_ptr self));
...
/**Function********************************************************************
  ...
  Description  [This method computes the set of <state,input>
                ...
                The strong backward image of S(X) is computed as
                follows.
						 ...
						 BddStates states)
{
  ...
  constr_trans = bdd_not(self->dd, states);  
  bdd_and_accumulate(self->dd, &constr_trans, self->invar_states);

  bdd_and_accumulate(self->dd, &constr_trans, self->invar_inputs);

  tmp = BddFsm_get_weak_backward_image(self, constr_trans); 
  bdd_free(self->dd, constr_trans);
  ...
  /* result is the set of state/input transitions that either originate
     from 'non-real states' or - in case there is at least one
     successor - are such that all the successors lay inside the set
     defined by the input parameter 'states' */     
  
  bdd_and_accumulate(self->dd, &result, self->invar_states);
  bdd_and_accumulate(self->dd, &result, self->invar_inputs);

  /* At this point, result is the set of state/input transitions that -
     in case there is at least one successor - are such that all the
     successors lay inside the set defined by the input parameter
     'states' */
     
  tmp = (bdd_ptr) bdd_fsm_get_backward_si_projection(self);
...
{  
  ...
  if (! BddFsm_is_total(self)) {
    ...
    BddEnc_print_bdd_begin(self->enc, 
       NodeList_to_node_ptr(Encoding_get_all_model_vars_list(self->senc)), 
       false);
...
/**Function********************************************************************
  ...
  Synopsis     [Returns the set of reachable states of this machine]
...
/**Function********************************************************************
  ...
  Synopsis     [Returns the image in terms of states and inputs from the 
  backward cluster list]

  Description  [Used for planning in strong backward image computation. 
  Could update the cache. Returned bdd is referenced.]
  
  SideEffects  [Cache can change]

******************************************************************************/
static BddStatesInputs bdd_fsm_get_backward_si_projection(BddFsm_ptr self)
{
  BddStatesInputs res;
  
  if ( CACHE_IS_EQUAL(backward_projection, BDD_STATES_INPUTS(NULL)) ) {
    bdd_ptr s = bdd_one(self->dd);
    
    /* here we use state instead of state_input since the list is backward, 
       and we need to quantify away next states, to obtain <X, I> */
    res = BDD_STATES_INPUTS(
		 BddTrans_get_backward_image_state_input(self->trans, s));

    CACHE_SET_BDD(backward_projection, res);
    bdd_free(self->dd, res);
    bdd_free(self->dd, s);
  } 

  res = CACHE_GET_BDD(backward_projection);
  return res;
}
