...
static char rcsid[] UTIL_UNUSED = "$Id: BddFsmCache.c,v 1.1.2.8 2004/04/16 15:07:59 nusmv Exp $";
...
BddFsmCache_ptr BddFsmCache_hard_copy(const BddFsmCache_ptr self)
{
  ...
  CACHE_COPY_BDD(BddStatesInputs, backward_projection);
...
void BddFsmCache_reset_not_reusable_fields_after_product(BddFsmCache_ptr self)
{
  ...
  CACHE_UNREF_BDD(BddStatesInputs, backward_projection);  
...
static void bdd_fsm_cache_init(BddFsmCache_ptr self, DdManager* dd)
{
  ...
  *(self->family_counter) = 1;
  ...
  self->backward_projection   = BDD_STATES_INPUTS(NULL);
