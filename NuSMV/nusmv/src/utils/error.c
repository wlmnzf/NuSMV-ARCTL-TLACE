...
static char rcsid[] UTIL_UNUSED = "$Id: error.c,v 1.8.2.14 2004/05/24 15:28:24 nusmv Exp $";
...
void warning_non_ag_only_spec(node_ptr spec) 
{
  fprintf(nusmv_stderr, "\n********   WARNING   ********\nThe ");
...
void error_init_simple_exp_is_not_ivar_only_expr(node_ptr prop)
{
  start_parsing_err();
  fprintf(nusmv_stderr, "SIMPLE_EXPR is not an IVAR-only expression:\n");
  Prop_print(prop, nusmv_stderr);
  finish_parsing_err();
}
