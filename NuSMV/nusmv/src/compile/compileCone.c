...
static char rcsid[] UTIL_UNUSED = "$Id: compileCone.c,v 1.11.4.12 2004/05/27 13:06:07 nusmv Exp $";
...
static Set_t
formulaGetDefinitionDependencies_ExeptForSimpleExpr ARGS((const Encoding_ptr senc, node_ptr formula));
...
//JVANDERM's FUNCTION
Set_t Formula_GetDependencies_ExeptForSimpleExpr(const Encoding_ptr senc, node_ptr formula, node_ptr context) {
	if (formula == Nil) {
	...
	case CONTEXT:
		return Formula_GetDependencies_ExeptForSimpleExpr(senc, cdr(formula), context);
	...
	case ATOM: {
		...
		if (((param != (node_ptr)NULL) && is_symb) ||(is_symb && (constant != (node_ptr)NULL))||((param
				!= (node_ptr) NULL) && (constant != (node_ptr) NULL))) {
			error_ambiguous(formula);
		}
		if (param != (node_ptr)NULL) {
			return Formula_GetDependencies_ExeptForSimpleExpr(senc, param, context);
		}
		if (constant != (node_ptr)NULL) {
			return Set_MakeEmpty();
		}
		/* it should be a defined symbol, running, or a variable */
		return formulaGetDefinitionDependencies_ExeptForSimpleExpr(senc, CompileFlatten_resolve_name(formula, context));
	}
	...
	case ARRAY: {
		...
		if (temp != (add_ptr)NULL) {
			...
			return formulaGetDefinitionDependencies_ExeptForSimpleExpr(senc, name);
		}
	}

	case CONS: {
		Set_t left = Formula_GetDependencies_ExeptForSimpleExpr(senc, car(formula), context);
		Set_t right = Formula_GetDependencies_ExeptForSimpleExpr(senc, cdr(formula), context);
	...
	case NEXT: {
		...
		return Formula_GetDependencies_ExeptForSimpleExpr(senc, car(formula), context);
	}
	...
	case CASE: {
		Set_t partial;
		Set_t condition = Formula_GetDependencies_ExeptForSimpleExpr(senc, car(car(formula)), context);
		Set_t then_arg = Formula_GetDependencies_ExeptForSimpleExpr(senc, cdr(car(formula)), context);
		Set_t else_arg = Formula_GetDependencies_ExeptForSimpleExpr(senc, cdr(formula), context);
...
//JVANDERM's FUNCTION
static Set_t formulaGetDefinitionDependencies_ExeptForSimpleExpr(const Encoding_ptr senc, node_ptr formula) {
	Set_t result = Set_MakeEmpty();
	...
	if (Encoding_is_symbol_var(senc, formula)) {
		...
		/* It is a define and the type must be CONTEXT */
		Set_t res = lookup_define_dep_hash(formula);
		if (res == BUILDING_DEP_SET) {
			error_circular(formula);
		}
		if (res != (Set_t) NULL) {
			/* dependencies of the defined symbol have been previously
			 computed */
			...
			insert_define_dep_hash(formula, BUILDING_DEP_SET);
			io_atom_push(formula);
			result = Formula_GetDependencies_ExeptForSimpleExpr(senc, def, Nil);
			io_atom_pop();
					 ...
					 node_ptr formula, node_ptr context)
{
  ...
  case EU:
  case AU:
  /* 01/08/11 sbusard: add supports for EW and AW CTL operators */
  case EW:
  case AW:
  	  {
        Set_t left = Formula_GetDependencies(senc, car(formula), context);
    ...
    /* Franco (02/08/2004): added support for EAX */
  case EAX:
    /* Franco (05/08/2004): addedd support for AAX and AAU and others */
  case AAX: 
  case EAF:
  case AAF:
  case EAG:
  case AAG:
    {
      return Formula_GetDependencies(senc, cdr(formula), context);
    }

    /* Franco (03/08/2004): added support for EAU */
  case EAU:
  case AAU:
  /* 04/08/11 sbusard: add supports for EAW and AAW CTL operators */
  case EAW:
  case AAW:
  	  {
          Set_t left = Formula_GetDependencies(senc, car(cdr(formula)), context);
          Set_t right = Formula_GetDependencies(senc, cdr(cdr(formula)), context);
...
static void coiInit(const Encoding_ptr senc)
{
  ...
  for (iter = NodeList_get_first_iter(vars); 
    ...
    node_ptr var = NodeList_get_value_at(vars, iter);
