...
static char rcsid[] UTIL_UNUSED = "$Id: compileFlatten.c,v 1.25.4.30.2.3 2004/07/19 11:54:09 nusmv Exp $";
...
static void 
instantiate ARGS((Encoding_ptr senc, node_ptr, node_ptr, node_ptr *, 
 ...
 node_ptr actual /* the actual module arguments */)
{
  ...
  compileFlattenHierarchy(senc, 
			  root_name, name, &trans_expr, &init_expr,
                          ...
                          &ltl_expr, &invars_expr, &justice_expr,
...
node_ptr CompileFlatten_expand_range(int a, int b)
{
  ...
  for(i=b ; i>=a ; i--) {
    res = cons(find_node(NUMBER, (node_ptr) i, Nil), res);
  }
		     ...
		     node_ptr context)
{
  ...
  case MODTYPE: {
      ...
      instantiate_by_name(senc, car(type), name, trans, init, invar, spec, 
			  compute, ltl_spec, invar_spec, justice, compassion, 
			  assign, procs, actual);
      break;
  }
  ...
  case PROCESS: {
    ...
    compileFlattenHierarchy(senc, pmod_name, name, trans, init, invar, spec,
			    compute, ltl_spec, invar_spec, justice, 
  ...
  case ARRAY: {
    ...
    for (i=dim1; i<=dim2; ++i) {
      ...
      instantiate_var(senc, find_node(ARRAY, name, index),
                      ...
                      invar_spec, justice, compassion, assign, procs, context);
    }
    break;
  }
		      ...
		      node_ptr *procs)
{
  ...
  while (iter != Nil) {
    ...
    instantiate_var(senc, name, type, trans, init, invar, spec, compute, 
		    ltl_spec, invar_spec, justice, compassion, assign, 
		    procs, mod_name);
  ...
  while (mod_body_decls != Nil) { /* loop again over module declaration */
    ...
    case ISA:
      instantiate_by_name(senc, car(cur_decl), mod_name, &tmp_trans, 
			  ...
			  &tmp_ltlspec, &tmp_invar_spec, &tmp_justice, 
				...
				node_ptr *procs, node_ptr actual)
{
  ...
  instantiate(senc, mod_def, name, trans, init, invar, spec, compute, ltl_spec,
              invar_spec, justice, compassion, assign, procs, actual);
 ...
 node_ptr actual /* the actual module arguments */)
{
  ...
  instantiate_by_name(senc, 
		      root_name, name, trans, init, invar, spec, compute,
                      ltl_spec, invar_spec, justice, compassion, &tmp_assign, 
		      procs, actual);
			 ...
			 node_ptr context)
{
  ...
  case EU:
  case AU:
  /* 01/08/11 sbusard: add AW and EW CTL operators */
  case EW:
  case AW:
    /* Franco (02/08/2004) : EAX operator added */
  case EAX:
  case AAX:
  case EAF:
  case AAF:
  ...
  case AAG:
    ...
    /* Franco (03/08/2004) : EAU operator added */
  case EAU:
  /* 04/08/11 sbusard: EAW operator */
  case EAW:
    {
      node_ptr left  = Flatten_FlattenSexpRecur(senc, car(sexp), context);
    ...
    /* Franco (03/08/2004) : AAU operator added */
  case AAU:
  /* 04/08/11 sbusard: AAW operator */
  case AAW:
    {
      node_ptr left  = Flatten_FlattenSexpRecur(senc, car(sexp), context);
      /* left is ctl_expr U ctl_expr*/
