...
static char rcsid[] UTIL_UNUSED = "$Id: compileCheck.c,v 1.5.2.24.2.1 2004/06/15 09:59:53 nusmv Exp $";
			  ...
			  node_ptr compassion_expr)
{
...

  {
    ...
    while (! ListIter_is_end(iter)) { /* Loops over variable list */
      node_ptr v = NodeList_get_value_at(list, iter);
...
NodeList_ptr Compile_get_expression_dependencies_ExeptForSimpleExpr(const Encoding_ptr senc, Expr_ptr expression) {
	Set_t dependencies = Formula_GetDependencies_ExeptForSimpleExpr(senc, (node_ptr) expression, Nil);
	node_ptr list = Set_Set2List(dependencies);
	return NodeList_create_from_list(list);
}
