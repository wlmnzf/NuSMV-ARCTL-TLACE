...
static char rcsid[] UTIL_UNUSED = "$Id: compileBEval.c,v 1.13.4.12 2004/05/26 07:58:34 nusmv Exp $";
				 ...
				 boolean in_next, boolean allow_nondet)
{
  ...
  case ARRAY: 
	...
	node_ptr name_info = Encoding_lookup_symbol(senc, name);
	if (Encoding_is_symbol_define(senc, name)) {
	  ...
	  return expr2bexpr_recur(enc, body, in_next, allow_nondet);
	}
	...
	if (Encoding_is_symbol_var(senc, name)) {
	  ...
	  if (in_next) return find_node(NEXT, name, Nil);
	  else return name;
	}
  ...
  case NEXT: 
    ...
    return expr2bexpr_recur(enc, car(expr), true, allow_nondet);
  ...
  case EQDEF:
	...
	if (Encoding_is_symbol_var(senc, name)) {
          if (Encoding_get_var_range(senc, name) == boolean_range) {
            ...
            return find_node(IFF, var, expr2bexpr_recur(enc, rhs, in_next, 
							allow_nondet));
          } else {
            /* scalar variable */
            nusmv_assert(!in_next);
            return scalar_atom2bexpr(enc, expr, in_next, allow_nondet);
          }
	} 
  ...
  case CONTEXT: 
    return expr2bexpr_recur(enc, cdr(expr), in_next, allow_nondet);
