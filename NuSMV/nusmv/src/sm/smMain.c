...
static char rcsid[] UTIL_UNUSED = "$Id: smMain.c,v 1.28.2.14.2.3 2004/08/02 08:39:43 nusmv Exp $";
  ...
  char * program, char * unknown_option)
{
  ...
  fprintf(nusmv_stderr,
                 "Usage:\t%s [-s] [-ctt] [-lp] [-n idx] [-v vl] "
	  "[-cpp] [-pre pp_list]\\\n", program); 

  fprintf(nusmv_stderr, "\t[-is] [-ic] [-ils] [-ii] [-r] [-f] [-int] [-h | -help] [-i iv_file]\\\n");
  fprintf(nusmv_stderr, "\t[-o ov_file] [-AG] [-load script_file] [-reorder] [-dynamic]\\\n");
  fprintf(nusmv_stderr, "\t[-m method] [[-mono]|[-thresh cp_t]|[-cp cp_t]|[-iwls95 cp_t]]\\\n");
  fprintf(nusmv_stderr, "\t[-coi] [-noaffinity] [-iwls95preorder] [-bmc] [-bmc_length k]\\\n");
  fprintf(nusmv_stderr, "\t[-ofm fm_file] [-obm bm_file] [input-file]\n");
  ...
  fprintf(nusmv_stderr, "\t -n idx\t\t specifies which property of SMV model should be checked\n");
...
# endif
  ...
  fprintf(nusmv_stderr, "\t -pre pp_list\t defines a space-separated list of pre-processors to run\n"
	  "\t\t\t (in the order given) on the input file. List must be in\n"
	  "\t\t\t double quotes if there is more than one pre-processor named.\n");
  if (get_preprocessors_num() > 0) {
    char* preps = get_preprocessor_names();
    fprintf(nusmv_stderr, "\t\t\t Available preprocessors are: %s\n", preps);
    FREE(preps);
  }
  else {
  ...
  fprintf(nusmv_stderr, "\t -load sc_file\t loads NuSMV commands from file \"sc_file\"\n");
  ...
  /* 29/04/10 sbusard */
    fprintf(nusmv_stderr, "\t -tlaces tl\t sets tlaces explanation level to \"tl\",\n");
    fprintf(nusmv_stderr, "\t\t\t value is used as bitmask\n");
    fprintf(nusmv_stderr,	"\t\t\t (1: ex; 2: eu; 4: eg, 0: activate)\n");
    fprintf(nusmv_stderr, "\t -otlaces file\t Prints counter-example to file \"file\"\n");

    /* 01/08/11 sbusard */
	fprintf(nusmv_stderr, "\t -dtlaces dl\t sets tlaces explanation depth to \"dl\"\n");
...
static void BannerPrint(FILE * file) 
{
  ...
  fprintf(file, "*** For more information of NuSMV see <http://nusmv.irst.itc.it>\n");
  ...
  fprintf(file, "WARNING*   This version has been modified by Franco Raimondi     WARNING\n");  
  fprintf(file, "WARNING*   The following operators have been added:              WARNING\n");  
  fprintf(file, "WARNING*   EAX simple_expr ctl_expr                              WARNING\n");  
  fprintf(file, "WARNING*   AAX simple_expr ctl_expr                              WARNING\n");  
  fprintf(file, "WARNING*   EAF simple_expr ctl_expr                              WARNING\n");  
  fprintf(file, "WARNING*   AAF simple_expr ctl_expr                              WARNING\n");  
  fprintf(file, "WARNING*   EAG simple_expr ctl_expr                              WARNING\n");  
  fprintf(file, "WARNING*   EA simple_expr [ ctl_expr U ctl_expr ]                WARNING\n");  
  fprintf(file, "WARNING*   AA simple_expr [ ctl_expr U ctl_expr ]                WARNING\n");  
  fprintf(file, "WARNING* (simple_expr is a condition on actions                  WARNING\n");  
  fprintf(file, "WARNING*                                                         WARNING\n");  
  fprintf(file, "WARNING*  NOTICE! THESE MODIFICATIONS MAY HAVE INTRODUCED BUGS   WARNING\n");  
  fprintf(file, "WARNING*  NOTICE - 2! THERE IS NO CHECK THAT simple_expr IS A    WARNING\n");  
  fprintf(file, "WARNING*  CONDITION ON ACTIONS! THIS CHECK *MUST* BE INTRODUCED  WARNING\n");  
  fprintf(file, "WARNING*  (SEE src/CHANGELOG)                                    WARNING\n");  
  fprintf(file, "WARNING*                                                         WARNING\n");  
  fprintf(file, "WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING  WARNING \n\n");  
...
# if HAVE_SOLVER_MINISAT  
  ...
  fprintf(file, "WARNING *** (see http://www.cs.chalmers.se/~een/Satzoo/).             ***\n");  
  ...
  fprintf(file, "WARNING *** Notice that minisat is for non-commercial purposes only.  ***\n");  
  fprintf(file, "WARNING *** NO COMMERCIAL USE OF MINISAT IS ALLOWED WITHOUT WRITTEN   ***\n");  
  fprintf(file, "WARNING *** PERMISSION FROM THE AUTHORS OF MINISAT.                   ***\n");  
  fprintf(file, "WARNING *** Please contact Niklas Een or Niklas Sorensson             ***\n");  
  fprintf(file, "WARNING *** {een,nik}@cs.chalmers.se for details.                     ***\n\n");
# endif
}
...
#   if HAVE_CPP
	...
	char* new_pp_list;
	new_pp_list = ALLOC(char, strlen(pp_list) + 4);
...
#   endif
	...
	if (strcmp(pp_list, "") == 0) {
	  ...
	  char new_pp_list[strlen(pp_list) + strlen(preprocessors) + 1];
...
# endif
    ...
    /* 29/04/10 sbusard */
	else if (strcmp(*argv,"-tlaces") == 0){
	  if (argc < 2) {
		fprintf(nusmv_stderr, "The \"-tlaces\" command line option requires an argument.\n");
		exit(1);
	  }
	  {
		char *err_occ[1];
		int cur_tlaces_level;

		err_occ[0] = "";
		argv++; argc -= 2;
		cur_tlaces_level = strtol(*(argv++),err_occ, 10);
		if (strcmp(err_occ[0], "") != 0) {
		  fprintf(nusmv_stderr, "Error: \"%s\" is not a valid value for the \"-tlaces\" command line option.\n", err_occ[0]);
		  exit(1);
		}

		if(cur_tlaces_level == 0)
			set_tlaces_explain_level_activate(options);
		if(cur_tlaces_level & TLACES_EXPLAIN_LEVEL_EX)
			set_tlaces_explain_level_ex(options);
		if(cur_tlaces_level & TLACES_EXPLAIN_LEVEL_EU)
			set_tlaces_explain_level_eu(options);
		if(cur_tlaces_level & TLACES_EXPLAIN_LEVEL_EG)
			set_tlaces_explain_level_eg(options);
	  }
	  continue;
	}
	else if (strcmp(*argv, "-otlaces") == 0){
	  argv++; argc -= 2;
	  set_tlaces_output_file(options, *(argv++));
	  continue;
	}
    /* 01/08/11 sbusard */
	else if (strcmp(*argv,"-dtlaces") == 0){
	  if (argc < 2) {
		fprintf(nusmv_stderr, "The \"-dtlaces\" command line option requires an argument.\n");
		exit(1);
	  }
	  {
		char *err_occ[1];
		...
		err_occ[0] = "";
		argv++; argc -= 2;
		cur_tlaces_depth = strtol(*(argv++),err_occ, 10);
		if (strcmp(err_occ[0], "") != 0) {
		  fprintf(nusmv_stderr, "Error: \"%s\" is not a valid value for the \"-dtlaces\" command line option.\n", err_occ[0]);
		  exit(1);
		}
