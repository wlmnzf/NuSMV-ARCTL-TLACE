...
static char rcsid[] UTIL_UNUSED = "$Id: bmcWff.c,v 1.12.4.5.2.2 2004/07/28 15:33:03 nusmv Exp $";
...
static node_ptr bmc_wff_mk_nnf(node_ptr wff, boolean pol)
{
  ...
  case TRUEEXP:                         /* TRUEEXP   */
    if (pol == true)
      return Bmc_Wff_MkTruth();
    else
      /* !1 <-> 0 */
      return Bmc_Wff_MkFalsity();
  case FALSEEXP:                        /* FALSEEXP  */
    if (pol == true)
      return Bmc_Wff_MkFalsity();
    else
  ...
  case NOT:                             /* NOT       */
    /* !(a) <-> (!a) */
    /* !(!a) <-> a */
    return bmc_wff_mk_nnf(car(wff), !pol);
  case AND:                             /* AND       */
    if (pol == true)
      return Bmc_Wff_MkAnd(bmc_wff_mk_nnf(car(wff), true),
			   bmc_wff_mk_nnf(cdr(wff), true));
    else
      /* !(a & b) <-> (!a | !b) */
      return Bmc_Wff_MkOr(bmc_wff_mk_nnf(car(wff), false),
       bmc_wff_mk_nnf(cdr(wff), false));
  case OR:                              /* OR        */
    if (pol == true)
      return Bmc_Wff_MkOr(bmc_wff_mk_nnf(car(wff), true),
       bmc_wff_mk_nnf(cdr(wff), true));
    else
      /* !(a | b) <-> (!a & !b) */
      return Bmc_Wff_MkAnd(bmc_wff_mk_nnf(car(wff), false),
        bmc_wff_mk_nnf(cdr(wff), false));
  ...
  case IFF:                             /* IFF       */
    if (pol == true)
	 ...
	 !(a & !b) & !(b & !a) <->
	 (!a | b) & (!b | a) */
      return Bmc_Wff_MkAnd( Bmc_Wff_MkOr(bmc_wff_mk_nnf(car(wff), false),
					 bmc_wff_mk_nnf(cdr(wff), true)),
	 ...
	 !(!(a & !b) & !(b & !a)) <->
	 (a & !b) | (b & !a) */
      return Bmc_Wff_MkOr( Bmc_Wff_MkAnd(bmc_wff_mk_nnf(car(wff), true),
					 bmc_wff_mk_nnf(cdr(wff), false)),
  ...
  case XOR:                             /* XOR       */
    if (pol == true)
      ...
      /* !(a xnor b) <-> (a | !b) & (!a | b) */
      return Bmc_Wff_MkAnd(Bmc_Wff_MkOr(bmc_wff_mk_nnf(car(wff), true),
                                        bmc_wff_mk_nnf(cdr(wff), false)),
  ...
  case OP_NEXT:                         /* OP_NEXT   */
    /* !X(a) <-> X(!a) */
    return Bmc_Wff_MkOpNext(bmc_wff_mk_nnf(car(wff), pol));

  case OP_PREC:                         /* OP_PREC   */
    /* !Y(a) <-> Z(!a) */
    if (pol == true)
      return Bmc_Wff_MkOpPrec(bmc_wff_mk_nnf(car(wff), pol));
    else
      return Bmc_Wff_MkOpNotPrecNot(bmc_wff_mk_nnf(car(wff), pol));
  ...
  case OP_GLOBAL:                       /* OP_GLOBAL */
    if (pol == true)
      return Bmc_Wff_MkGlobally(bmc_wff_mk_nnf(car(wff), pol));
    else
      /* !G(a) <-> F(!a) */
      return Bmc_Wff_MkEventually(bmc_wff_mk_nnf(car(wff), pol));

  case OP_HISTORICAL:                       /* OP_HISTORICAL */
    if (pol == true)
      return Bmc_Wff_MkHistorically(bmc_wff_mk_nnf(car(wff), pol));
    else
      /* !H(a) <-> O(!a) */
      return Bmc_Wff_MkOnce(bmc_wff_mk_nnf(car(wff), pol));
  ...
  case UNTIL:                           /* UNTIL     */
    if (pol == true)
      return Bmc_Wff_MkUntil(bmc_wff_mk_nnf(car(wff), pol),
          bmc_wff_mk_nnf(cdr(wff), pol));
    else
      /* !(a U b) <-> (!a V !b) */
      return Bmc_Wff_MkReleases(bmc_wff_mk_nnf(car(wff), pol),
             bmc_wff_mk_nnf(cdr(wff), pol));

  case SINCE:                           /* SINCE     */
    if (pol == true)
      return Bmc_Wff_MkSince(bmc_wff_mk_nnf(car(wff), pol),
          bmc_wff_mk_nnf(cdr(wff), pol));
    else
      /* !(a S b) <-> (!a T !b) */
      return Bmc_Wff_MkTriggered(bmc_wff_mk_nnf(car(wff), pol),
             bmc_wff_mk_nnf(cdr(wff), pol));
  ...
  case CASE:
    {
      ...
      return bmc_wff_mk_nnf(nocase_wff, pol);
    }
  ...
  case ARRAY:  
    /* is a bexp var */
    if (pol == true) return wff;
    else return Bmc_Wff_MkNot(wff);
