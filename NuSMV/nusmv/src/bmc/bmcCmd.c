...
static char rcsid[] UTIL_UNUSED = "$Id: bmcCmd.c,v 1.85.4.5.2.16 2004/08/04 16:26:01 nusmv Exp $";
			 ...
			 char** res_o)
{
  ...
  if (str_loop != (char*) NULL) {
    ...
    if (res != SUCCESS) {
      bmcErrorInvalidNumber(str_loop);
      FREE(str_loop);
      return GENERIC_ERROR;
    }
    FREE(str_loop);
	...
	!= SUCCESS) {
      bmcErrorInvalid_k_l(*res_k, rel_loop);
      return GENERIC_ERROR;
    }
  ...
  if (str_formula != (char*) NULL) {
    ...
    if (idx == -1) {
      bmcErrorInvalidProperty(str_formula);
      FREE(str_formula);
      return GENERIC_ERROR;
    }
...
/**Function********************************************************************
  ...
  Synopsis           [Private service for error reporting]
