...
static char rcsid[] UTIL_UNUSED = "$Id: bmcBmcNonInc.c,v 1.1.2.12 2004/08/04 16:32:43 nusmv Exp $";
  ...
  {   /* generates the problem: */
	...
	for (i = 0; i < k ; i++) {
	  for (j = 0; j < i; j++) {
	    ...
	    first_var_idx = BmcVarsMgr_GetFirstCurrStateVarIndex(vars_mgr);
	    for (v = first_var_idx; 
		 v < first_var_idx + BmcVarsMgr_GetStateVarsNum(vars_mgr); 
		 v++) {
	      be_ptr be_var = BmcVarsMgr_VarIndex2Curr(vars_mgr, v);
	      be_ptr be_xor =
		Be_Xor(be_mgr,
